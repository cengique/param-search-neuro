...
# This SGE job script reads a designated row from the parameter file
# and executes genesis to process it. It uses a fast hashtable access
# to read the parameter row from a database created with the par2db.pl
# script.  Notice that, this script does not mark the row as processed
# in the original parameter file. One needs to use checkMissing.pl
# script to find out which rows are already done.  The reason this
# script does not modify the parameter file is to avoid race
# conditions that occur when writing to the parameter file
# concurrently.
...
# Author: Cengiz Gunay <cgunay@emory.edu> 2005/06/29, 2015/05/26
